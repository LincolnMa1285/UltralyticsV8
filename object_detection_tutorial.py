...
class ObjectDetector:
    ...
    def predict_image(self, source, conf=0.25, iou=0.7, save=True, show=False):
        ...
        """
        print(f"\n开始图像检测: {source}")
        results = self.model.predict(source=source, conf=conf, iou=iou, save=save, show=show, device=self.device)
    ...
    def predict_video(self, source, conf=0.25, iou=0.7, save=True, show=False):
        ...
        results = self.model.predict(
            source=source, conf=conf, iou=iou, save=save, show=show, stream=True, device=self.device
        )
    ...
    def predict_webcam(self, source=0, conf=0.25, iou=0.7):
        ...
        """
        print("\n启动摄像头检测 (按 'q' 键退出)")
        results = self.model.predict(source=source, conf=conf, iou=iou, show=True, stream=True, device=self.device)
    ...
    def train(self, data, epochs=100, imgsz=640, batch=16, project=None, name="exp"):
        ...
        results = self.model.train(
            data=data, epochs=epochs, imgsz=imgsz, batch=batch, device=self.device, project=project, name=name
        )
    ...
    def validate(self, data=None, split="val", imgsz=640):
        ...
        """
        print("\n开始验证模型")
        metrics = self.model.val(data=data, split=split, imgsz=imgsz, device=self.device)
    ...
    def export(self, format="onnx", imgsz=640, half=False, simplify=True):
        ...
        export_path = self.model.export(format=format, imgsz=imgsz, half=half, simplify=simplify)
...
def main():
    ...
    parser.add_argument("--model", type=str, default="yolov8n.pt", help="模型权重文件路径")
    parser.add_argument(
        "--source", type=str, default=str(ASSETS / "bus.jpg"), help="输入源 (图像路径、视频路径、摄像头编号)"
    )
    parser.add_argument("--data", type=str, default="coco8.yaml", help="数据集配置文件路径 (训练/验证时使用)")
    parser.add_argument("--conf", type=float, default=0.25, help="置信度阈值")
    parser.add_argument("--iou", type=float, default=0.7, help="NMS IOU 阈值")
    parser.add_argument("--device", type=str, default="", help="运行设备 (cpu, cuda, mps 等)")
    parser.add_argument("--epochs", type=int, default=100, help="训练轮数")
    parser.add_argument("--imgsz", type=int, default=640, help="输入图像尺寸")
    parser.add_argument("--batch", type=int, default=16, help="批次大小")
    parser.add_argument(
        "--format", type=str, default="onnx", help="导出格式 (onnx, torchscript, openvino, engine, coreml, tflite 等)"
    )
    parser.add_argument("--save", action="store_true", default=True, help="是否保存结果")
    ...
    if args.mode == "predict":
        detector.predict_image(source=args.source, conf=args.conf, iou=args.iou, save=args.save, show=args.show)

    elif args.mode == "video":
        detector.predict_video(source=args.source, conf=args.conf, iou=args.iou, save=args.save, show=args.show)

    elif args.mode == "webcam":
        source = int(args.source) if args.source.isdigit() else args.source
        detector.predict_webcam(source=source, conf=args.conf, iou=args.iou)

    elif args.mode == "train":
        detector.train(data=args.data, epochs=args.epochs, imgsz=args.imgsz, batch=args.batch)

    elif args.mode == "validate":
        detector.validate(data=args.data, imgsz=args.imgsz)

    elif args.mode == "export":
        detector.export(format=args.format, imgsz=args.imgsz)
